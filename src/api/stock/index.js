...
router.get('/:id', function(req, res, next) {
  Drug.findById(req.params.id, function (err, post) {
    if (err) {
...
router.put('/:id', function(req, res, next) {
  Drug.findByIdAndUpdate(req.params.id, req.body, function (err, post) {
    if (err) {
...
router.delete('/:id', function(req, res, next) {
  Drug.findByIdAndRemove(req.params.id, req.body, function (err, post) {
    if (err) return next(err);
    res.json({status: 200, content: post});
  });
});
